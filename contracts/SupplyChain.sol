...
contract SupplyChain {
  ...
  modifier forSale(uint sku) {
    nstate = State.ForSale;
    require(items[sku].state == nstate && items[sku].buyer == address(0));
    _;
  }
  
  modifier sold(uint sku) {
    nstate = State.Sold;
    require(items[sku].state == nstate);
    _;
  }
